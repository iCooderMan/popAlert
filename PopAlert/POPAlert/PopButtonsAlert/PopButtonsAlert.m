...
//#define COLOR_HEX(X) COLOR_WITH_HEX(X)
#define COLOR_HEX(X) [UIColor blackColor]

@interface PopButtonsAlert ()
...
@implementation PopButtonsAlert
...
//    textLabel.font          = [UIFont HeitiSCWithFontSize:15.f];
    ...
    if (buttonsInfo.count == 1) {
        ...
        [self.messageView addSubview:[[self class] lineViewWithFrame:CGRectMake(0, self.messageView.height - 40, self.messageView.width, 0.3f) color:COLOR_HEX(@"#B9B9B9")]];
...
//        self.firstButton.titleLabel.font        = [UIFont HeitiSCWithFontSize:16.f];
    ...
    if (buttonsInfo.count == 2) {
        ...
        [self.messageView addSubview:[[self class] lineViewWithFrame:CGRectMake(0, self.messageView.height - 40, self.messageView.width, 0.3f)
                                                               color:COLOR_HEX(@"#B9B9B9")]];
        [self.messageView addSubview:[[self class] lineViewWithFrame:CGRectMake(self.messageView.width / 2.f, self.messageView.height - 40, 0.3f, 40.f)
                                                               color:COLOR_HEX(@"#B9B9B9")]];
        
        self.firstButton.frame          = CGRectMake(self.messageView.width / 2.f, self.messageView.height - 40, self.messageView.width / 2.f, 40);
        self.firstButton.exclusiveTouch = YES;
        [self.firstButton setTitle:self.buttonsTitle[0] forState:UIControlStateNormal];
        [self.firstButton setTitleColor:self.titleColors[0] forState:UIControlStateNormal];
        [self.messageView addSubview:self.firstButton];

        
        self.secondButton.frame          = CGRectMake(0, self.messageView.height - 40, self.messageView.width / 2.f, 40);
        self.secondButton.exclusiveTouch = YES;
        [self.secondButton setTitle:self.buttonsTitle[1] forState:UIControlStateNormal];
        [self.secondButton setTitleColor:self.titleColors[1] forState:UIControlStateNormal];
